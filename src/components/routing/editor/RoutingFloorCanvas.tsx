...
import { MapNode, NodeType } from '@/lib/routing/graph/types'
import {
...
export default function RoutingFloorCanvas() {
  ...
  const handleZoomIn = useCallback(() => {
    ...
    const next = Math.min(ZOOM_MAX, parseFloat((s.zoom + ZOOM_STEP).toFixed(2)))
    s.zoom = next
    setZoomLevel(next)
  }, [])
  ...
  const handleZoomOut = useCallback(() => {
    const s = stateRef.current
    const next = Math.max(ZOOM_MIN, parseFloat((s.zoom - ZOOM_STEP).toFixed(2)))
    s.zoom = next
    setZoomLevel(next)
  }, [])
  ...
  const draw = useCallback(() => {
    ...
    if (!ctx) return
    const { panOffsetX, panOffsetY, edgeStartNodeId: esn, mouseX, mouseY, hoverNodeId, hoverEdgeId, zoom } = stateRef.current
  ...
  return (
    <div ref={containerRef} style={{ flex: 1, position: 'relative', overflow: 'hidden', cursor: getCursor(activeTool) }}>
      ...
      <div style={{
        ...
        {edgeStartNodeId && <span style={{ marginLeft: 8, color: 'var(--routing-primary-light)' }}>● Connecting…</span>}
      </div>
      ...
      <div style={{
        ...
        border: '1px solid var(--routing-border)', pointerEvents: 'none',
      }}>
