...
export function FloatingPanel({ 
  ...
  return (
    ...
    >
      ...
      >
        <div className="flex items-center gap-2 text-slate-800 font-medium text-sm">
          <span className="text-slate-500 flex items-center justify-center">
            {minimized ? <ChevronRight className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
          </span>
      ...
      {/* Content Area */}
      <div 
        className={`transition-all duration-300 ease-in-out ${minimized ? 'max-h-0 opacity-0 overflow-hidden' : 'max-h-[70vh] opacity-100 overflow-y-auto'}`}
      >
        <div className="p-3">
          {children}
        </div>
