...
export default function MapPage() {
  ...
  return (
    <div className="fixed inset-0 z-[60] bg-slate-50 overflow-hidden flex flex-col font-sans" style={{ background: 'var(--routing-bg-base)' }}>
      <MapIncidentBridge />
      ...
      <div className="absolute top-4 left-4 z-[75]">
        <Link href="/dashboard" className="px-4 py-2 bg-slate-900 text-white rounded-lg font-medium text-sm shadow-xl hover:bg-slate-800 transition-colors inline-block w-max">
      ...
      <div className="absolute top-4 left-1/2 -translate-x-1/2 z-[75] flex flex-col items-center gap-2">
        <div className="bg-white/95 backdrop-blur-md border border-slate-200 shadow-xl rounded-xl p-2 flex flex-col sm:flex-row items-center gap-3">
          <div className="flex gap-1">
            <button onClick={() => setActiveTab("editor")} className={`px-4 py-1.5 rounded-lg text-sm font-semibold transition-all border ${activeTab === "editor" ? 'bg-sky-100 text-sky-700 border-sky-300' : 'bg-transparent text-slate-500 border-transparent hover:bg-slate-100'}`}>
              ✏️ Map Editor
            </button>
            <button onClick={() => setActiveTab("navigate")} className={`px-4 py-1.5 rounded-lg text-sm font-semibold transition-all border ${activeTab === "navigate" ? 'bg-sky-100 text-sky-700 border-sky-300' : 'bg-transparent text-slate-500 border-transparent hover:bg-slate-100'}`}>
              🧭 Navigate & Route
            </button>
          </div>
          ...
          {mounted && (
            <div className="flex gap-2 border-l border-slate-200 pl-3">
              <span className="px-3 py-1.5 bg-red-100 text-red-700 rounded-md text-xs font-bold border border-red-200">🔥 {criticalCount()} Critical</span>
              <span className="px-3 py-1.5 bg-slate-800 text-slate-300 rounded-md text-xs font-bold">🚨 {activeCount()} Active</span>
            </div>
          )}
        </div>
      </div>
      ...
      {activeTab === "editor" && (
        <>
          <FloatingPanel title="Floor Manager" position="bottom-left" icon="🏢" defaultMinimized={false}>
            <RoutingFloorManager />
          </FloatingPanel>
          <FloatingPanel title="Map Drawing Tools" position="top-right" icon="🛠️" width={180}>
            <RoutingToolbar />
          </FloatingPanel>
          <FloatingPanel title="Node Properties" position="bottom-right" icon="⚙️" width={260}>
            <RoutingNodePanel />
            <div className="mt-4"><RoutingEdgePanel /></div>
          </FloatingPanel>
        </>
      )}
      ...
      {activeTab === "navigate" && (
        <>
          <FloatingPanel title="Routing Controls" position="bottom-left" icon="🎮" width={280}>
            <RoutingPathControls />
          </FloatingPanel>
          <FloatingPanel title="Step-by-Step Directions" position="top-right" icon="🗺️" width={320}>
            <RoutingStepList />
          </FloatingPanel>
        </>
      )}
      ...
      {mounted && (activeEmergencies.length > 0 || recentIncidents.length > 0) && (
        <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-[70] bg-white/95 backdrop-blur-md border border-slate-200 shadow-2xl rounded-xl overflow-hidden min-w-[500px]">
          <div className="px-5 py-3 border-b border-slate-100 bg-red-50/50 flex items-center gap-3">
            ...
            <span className="text-xs font-bold text-red-500 tracking-wider uppercase">Live Alerts · {activeEmergencies.length} Active</span>
          </div>
          <div className="p-4 flex gap-3 flex-wrap items-center">
            {activeEmergencies.map(({ nodeId, type, node, floor }) => (
              <div key={nodeId} className="flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-semibold" style={{ background: `rgba(${type === 'fire' ? '239,68,68' : type === 'medical' ? '249,115,22' : type === 'hazmat' ? '168,85,247' : type === 'security' ? '234,179,8' : '148,163,184'},0.15)`, color: EMERGENCY_COLORS[type] ?? '#94A3B8' }}>
            ...
            {activeEmergencies.length === 0 && (
              <span className="text-xs text-slate-500">No active map emergencies automatically created yet.</span>
            )}
          </div>
        </div>
      )}
